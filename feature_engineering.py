...
def bin_to_num(data):
    binnedinc = []
    original_column_name = 'binnedInc'
    for i in data[original_column_name]:
        i = i.strip("()[]")
        i = i.split(",")
        i = tuple(i)
        i = tuple(map(float, i))
        i = list(i)
        binnedinc.append(i)
    data[original_column_name] = binnedinc
    data["lower_bound"] = [i[0] for i in data[original_column_name]]
    data["upper_bound"] = [i[1] for i in data[original_column_name]]
